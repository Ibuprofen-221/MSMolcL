...
from services.retrieve_advanced import init_retrieve_advanced_process
from services.retrieve_advanced_service import execute_retrieve_advanced_job

DEFAULT_ION_MODE = "pos"
VALID_ION_MODES = ("pos", "neg")

RETRIEVE_ADVANCED_MAX_PENDING_JOBS = int(getattr(app_cfg, "retrieve_advanced_max_pending_jobs", 256))
RETRIEVE_ADVANCED_MP_START_METHOD = str(getattr(app_cfg, "retrieve_advanced_mp_start_method", "spawn"))
RETRIEVE_ADVANCED_QUEUE_WAIT_TIMEOUT = float(getattr(app_cfg, "retrieve_advanced_queue_wait_timeout", 0.2))
RETRIEVE_ADVANCED_TASK_KEEP_SECONDS = int(getattr(app_cfg, "retrieve_advanced_task_keep_seconds", 3600))
...
def _gpu_worker_main(request_queue, response_queue, ion_mode: str) -> None:
    try:
        init_retrieve_advanced_process(ion_mode=ion_mode)
    except Exception as exc:
        response_queue.put(
            {
                "type": "worker_error",
                "ion_mode": ion_mode,
                "error": f"高级检索GPU进程模型初始化失败({ion_mode}): {exc}",
                "at": _utc_now_iso(),
            }
        )
        return
    ...
    while True:
        ...
        try:
            result = execute_retrieve_advanced_job(job_payload)
            response_queue.put(
                {
                    "type": "done",
                    "job_id": job_id,
...
class RetrieveAdvancedRuntime:
    ...
    async def startup(self) -> None:
        ...
        mp_ctx = multiprocessing.get_context(RETRIEVE_ADVANCED_MP_START_METHOD)

        for mode in VALID_ION_MODES:
            request_queue = mp_ctx.Queue(maxsize=RETRIEVE_ADVANCED_MAX_PENDING_JOBS)
            response_queue = mp_ctx.Queue(maxsize=RETRIEVE_ADVANCED_MAX_PENDING_JOBS * 2)

            process = mp_ctx.Process(
                target=_gpu_worker_main,
                args=(request_queue, response_queue, mode),
                daemon=False,
                name=f"retrieve-advanced-gpu-worker-{mode}",
            )
            process.start()
            ...
            consumer_thread = Thread(
                ...
                args=(mode,),
                daemon=True,
                name=f"retrieve-advanced-response-consumer-{mode}",
            )
    ...
    async def shutdown(self) -> None:
        ...
        for proc in self._worker_processes.values():
            proc.join(timeout=8)
            if proc.is_alive():
                proc.terminate()
                proc.join(timeout=2)

        for th in self._consumer_threads.values():
            if th.is_alive():
                th.join(timeout=2)
        ...
        self._mark_active_jobs_failed("高级检索服务已关闭")
    ...
    def submit(self, job_payload: dict, task_id: str) -> dict:
        ...
        if queue is None or proc is None or not proc.is_alive():
            raise RuntimeError(f"{ion_mode} 离子高级检索执行器尚未初始化")
        ...
        with self._lock:
            ...
            if active_jobs >= RETRIEVE_ADVANCED_MAX_PENDING_JOBS:
                raise RuntimeError(f"{ion_mode} 离子高级检索任务过多，请稍后重试")
            ...
            self._jobs[job_id] = {
                ...
                "task_id": task_id,
                "ion_mode": ion_mode,
                "worker_key": f"advanced-{ion_mode}",
                "status": "pending",
                "created_at": now,
                "started_at": None,
                "finished_at": None,
        ...
        try:
            ...
            raise RuntimeError(f"{ion_mode} 离子高级检索任务过多，请稍后重试") from None
    ...
    def _consume_responses(self, ion_mode: str) -> None:
        while not self._stop_event.is_set():
            ...
            try:
                message = response_queue.get(timeout=RETRIEVE_ADVANCED_QUEUE_WAIT_TIMEOUT)
            except Empty:
                continue
            except Exception:
                continue
    ...
    def _handle_worker_message(self, message: dict, ion_mode: str) -> None:
        ...
        if msg_type == "worker_error":
            error = str(message.get("error") or f"高级检索GPU进程异常({ion_mode})")
    ...
    def _sync_worker_state(self) -> None:
        for ion_mode, proc in self._worker_processes.items():
            ...
            if self._worker_errors.get(ion_mode) is None and not self._stop_event.is_set():
                error = f"高级检索GPU进程异常退出({ion_mode})"
                self._worker_errors[ion_mode] = error
                self._mark_active_jobs_failed(error, ion_mode=ion_mode)
    ...
    def _cleanup_finished_locked(self) -> None:
        expire_at = datetime.utcnow() - timedelta(seconds=RETRIEVE_ADVANCED_TASK_KEEP_SECONDS)
        with self._lock:
            expired = []
...
retrieve_advanced_runtime = RetrieveAdvancedRuntime()
